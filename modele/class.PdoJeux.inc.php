...
class PdoJeux
{
     ...
     */
    public function ajouterPegis(string $ageLimite): int
    {
        try {
            $requete_prepare = PdoJeux::$monPdo->prepare("INSERT INTO pegi "
                . "(idPegi, ageLimite) "
                . "VALUES (0, :unLibPegi) ");
            $requete_prepare->bindParam(':unLibPegi', $ageLimite, PDO::PARAM_STR);
            $requete_prepare->execute();
            // récupérer l'identifiant crée
            return PdoJeux::$monPdo->lastInsertId();
        } catch (Exception $e) {
     ...
     * Modifie le libellé du genre donné en paramètre
     * 
     * @param int $idPegi : l'identifiant du genre à modifier  
     ...
     * Supprime le genre donné en paramètre
     * 
     * @param int $idPegi :l'Descriptiobn du pegi à supprimer 
     */
    public function getDescrption(): array
    {
        $requete = 'SELECT idPegi as identifiant, descPegi as libelle 
						FROM pegi 
						ORDER BY descPegi';
        try {
            $resultat = PdoJeux::$monPdo->query($requete);
